...
<div :class="formControls ? _class : ''">
    ...
    <div v-show="formControls">
        ...
        <div class="input-group">
            <select-document-set-dropdown
                :id="id + '-select-document-set-dropdown'"
                :class="error[0] ? 'multiselect-invalid-feedback' : ''"
                openDirection="below"
                placeholder=""
                :options="$root.documentSets" 
                v-model="record"
                track-by="recordid"
                :tabindex="-1"
                label="description"
                ...
                :allow-empty="false"
                :show-labels="false"
                @input="documentSetSelected"
            >
            <template slot="noResult">No Document Sets found</template>
            <template slot="noOptions">No Document Sets found...</template>
...
export default {
    ...
    components: {
        ...
        SelectDocumentSetDropdown: Multiselect
    },

    props: {
        clearIconCallback: { type: String, default: 'clearSelectDocumentSet' },
        clearIcon: { type: Boolean, default: false },
        title: {
            type: String,
            default: 'Click on the button to select a Document Set',
        }, 
        _class: {
            type: String, 
            default: 'col-md-12',
    ...
    mounted () {
        this.$parent.selectDocumentSet = this;
    },    
    ...
    methods: {
        ...
        documentSetSelected(selectedRecord) {
            ...
            if ( typeof this.$parent.documentSetSelected === 'function') this.$parent.documentSetSelected(selectedRecord);

        },        

        selectRecord(id) {

            this.documentSetSelected( this.table.table.row('#' + id).data() );
