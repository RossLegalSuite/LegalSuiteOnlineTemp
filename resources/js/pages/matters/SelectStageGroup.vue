    ...
    <div v-show="formControls">
        ...
        <div class="input-group">
            <select-stage-group-dropdown
                :id="id + '-select-stage-group-dropdown'"
                :class="error[0] ? 'multiselect-invalid-feedback' : ''"
                openDirection="below"
                placeholder=""
                :options="$root.stageGroups" 
                v-model="record"
                track-by="recordid"
                :tabindex="-1"
                label="description"
                ...
                :allow-empty="false"
                :show-labels="false"
                @input="stageGroupSelected"
            >
            <template slot="noResult">No Stage Groups found</template>
            <template slot="noOptions">No Stage Groups found...</template>
...
export default {
    ...
    components: {
        StageGroupTable: () => import("./StageGroupTable"),
        SelectStageGroupDropdown: Multiselect
    },

    props: {
        clearIconCallback: { type: String, default: 'clearSelectStageGroup' },
    ...
    methods: {
        ...
        stageGroupSelected(selectedRecord) {
