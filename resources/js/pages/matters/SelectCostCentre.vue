...
<div :class="formControls ? _class : ''">
    ...
    <div v-show="formControls">
        ...
        <div class="input-group">
            <select-cost-centre-dropdown
                :id="id + '-select-cost-centre-dropdown'"
                :class="error[0] ? 'multiselect-invalid-feedback' : ''"
                openDirection="below"
                placeholder=""
                :options="$root.costCentres" 
                v-model="record"
                track-by="recordid"
                :tabindex="-1"
                label="description"
                ...
                :allow-empty="false"
                :show-labels="false"
                @input="costCentreSelected"
            >
            <template slot="noResult">No Cost Centres found</template>
            <template slot="noOptions">No Cost Centres found...</template>
...
export default {
    ...
    components: {
        CostCentreTable: () => import("./CostCentreTable"),
        SelectCostCentreDropdown: Multiselect
    },

    props: {
        clearIconCallback: { type: String, default: 'clearSelectCostCentre' },
        clearIcon: { type: Boolean, default: false },
        title: {
            type: String,
            default: 'Click on the button to select a Cost Centre',
        }, 
        _class: {
            type: String, 
            default: 'col-md-12',
    ...
    mounted () {
        this.$parent.selectCostCentre = this;
    },    
    ...
    methods: {
        ...
        costCentreSelected(selectedRecord) {
            ...
            if ( typeof this.$parent.costCentreSelected === 'function') this.$parent.costCentreSelected(selectedRecord.recordid, selectedRecord.description);
        ...
        selectRecord(id) {
            ...
            this.costCentreSelected( this.table.table.row('#' + id).data() );
