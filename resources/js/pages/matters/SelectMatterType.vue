...
<div :class="formControls ? _class : ''">
    ...
    <div v-show="formControls">
        ...
        <div class="input-group">
            <select-matter-type-dropdown
                :id="id + '-select-matter-type-dropdown'"
                :class="error[0] ? 'multiselect-invalid-feedback' : ''"
                openDirection="below"
                placeholder=""
                :options="$root.matterTypes" 
                v-model="record"
                track-by="recordid"
                :tabindex="-1"
                label="description"
                ...
                :allow-empty="false"
                :show-labels="false"
                @input="matterTypeSelected"
            >
            <template slot="noResult">No Matter Types found</template>
            <template slot="noOptions">No Matter Types found...</template>
...
export default {
    ...
    components: {
        MatterTypeTable: () => import("./MatterTypeTable"),
        SelectMatterTypeDropdown: Multiselect
    },

    props: {
        clearIconCallback: { type: String, default: 'clearSelectMatterType' },
        clearIcon: { type: Boolean, default: false },
        title: {
            type: String,
            default: 'Click on the button to select a Matter Type',
        }, 
        _class: {
            type: String, 
            default: 'col-md-12',
    ...
    mounted () {
        this.$parent.selectMatterType = this;
    },    
    ...
    methods: {
        ...
        matterTypeSelected(selectedRecord) {
            ...
            if ( typeof this.$parent.matterTypeSelected === 'function') this.$parent.matterTypeSelected(selectedRecord);

        },        

        selectRecord(id) {

            this.matterTypeSelected( this.table.table.row('#' + id).data() );
