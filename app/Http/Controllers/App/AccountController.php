...
class AccountController extends Controller {
    ...
    public function get(Request $request)
    {
        ...
        if ($request->id) {
            ...
            if (!empty($account->children)) $account->noEdit = 1;
            ...
            if ($request->dataFlag === 'topLevelOnly') $query->whereNull( 'accounts.parentId' );            

            if ($request->parentId) $query->where('accounts.parentId',$request->parentId);

            if ($request->code) $query->where('accounts.code',$request->code);
    ...
    protected function generatetNextAccountCode($code, $parentId = null)
    {
        ...
        if ( !$parentId ) $query->whereNull('parentId');
