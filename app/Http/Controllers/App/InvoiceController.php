...
class InvoiceController extends Controller {
    ...
    public function get(Request $request)
    {
        ...
        if ($request->partyId) $query->where('client.id',$request->partyId);

        if ($request->matterId) $query->where('invoices.matterId',$request->matterId);

        if ($request->parentId) $query->where('invoices.matterId',$request->parentId);

        if ($request->createdById) $query->where('invoices.createdById',$request->createdById);

        if ($request->posted) $query->where('posted',1);

        if ($request->id) $query->where('invoices.id',$request->id);
