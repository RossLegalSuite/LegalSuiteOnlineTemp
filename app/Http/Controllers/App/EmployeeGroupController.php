...
class EmployeeGroupController extends Controller {
    ...
    public function get(Request $request)
    {
        ...
        if ($request->id) $query->where('employee_groups.id',$request->id);

        if ($request->description) $query->where('employee_groups.description',$request->description);

        if ($request->code) $query->where('employee_groups.code',$request->code);        
    ...
    public function store(Request $request)
    {
        ...
        if ( isset($request->id) )$rules['code'] = ['required',Rule::unique('employee_groups')->ignore($request->id)];
