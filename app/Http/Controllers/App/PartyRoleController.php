...
class PartyRoleController extends Controller {
    ...
    public function get(Request $request)
    {
        ...
        if ($request->id) $query->where('party_roles.id',$request->id);

        if ($request->code) $query->where('party_roles.code',$request->code);

        if ($request->description) $query->where('party_roles.description',$request->description);
    ...
    public function store(Request $request)
    {
        ...
        if ( isset($request->id) )$rules['code'] = ['required',Rule::unique('party_roles')->ignore($request->id)];
