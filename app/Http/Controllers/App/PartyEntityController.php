...
class PartyEntityController extends Controller
{
    public function get(Request $request)
    {
        ...
        if ($request->id) {
            $query->where('party_entities.id', $request->id);
        }

        if ($request->code) {
            $query->where('party_entities.code', $request->code);
        }

        if ($request->description) {
            $query->where('party_entities.description', $request->description);
        }
    ...
    public function store(Request $request)
    {
        ...
        if (isset($request->id)) {
            $rules['code'] = ['required', Rule::unique('party_entities')->ignore($request->id)];
        }
