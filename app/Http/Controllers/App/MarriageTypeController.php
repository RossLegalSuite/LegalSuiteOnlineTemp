...
class MarriageTypeController extends Controller
{
    public function get(Request $request)
    {
        ...
        if ($request->id) {
            $query->where('marriage_types.id', $request->id);
        }

        if ($request->description) {
            $query->where('marriage_types.description', $request->description);
        }

        if ($request->code) {
            $query->where('marriage_types.code', $request->code);
        }
    ...
    public function store(Request $request)
    {
        ...
        if (isset($request->id)) {
            $rules['code'] = ['required', Rule::unique('marriage_types')->ignore($request->id)];
        }
