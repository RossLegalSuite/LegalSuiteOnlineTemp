...
class JournalTransactionController extends Controller
{
    ...
    public function get(Request $request)
    {
        ...
        if ($request->id) {
            $query->where('journal_transactions.id', $request->id);
        }

        if ($request->parentId) {
            $query->where('journal_transactions.journalId', $request->parentId);
        }

        if ($request->accountId) {
            $query->where('journal_transactions.accountId', $request->accountId);
        }
