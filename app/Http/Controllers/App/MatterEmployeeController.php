...
class MatterEmployeeController extends Controller
{
    ...
    public function get(Request $request)
    {
        ...
        if ($request->id) {
            $query->where('matter_employees.id', $request->id);
        }

        if ($request->matterId) {
            $query->where('matter_employees.matterId', $request->matterId);
        }

        if ($request->employeeId) {
            $query->where('matter_employees.employeeId', $request->employeeId);
        }

        if ($request->roleId) {
            $query->where('matter_employees.roleId', $request->roleId);
        }
