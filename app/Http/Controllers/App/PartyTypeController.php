...
class PartyTypeController extends Controller
{
    public function get(Request $request)
    {
        ...
        if ($request->id) {
            $query->where('party_types.id', $request->id);
        }

        if ($request->code) {
            $query->where('party_types.code', $request->code);
        }

        if ($request->description) {
            $query->where('party_types.description', $request->description);
        }
    ...
    public function store(Request $request)
    {
        ...
        if (isset($request->id)) {
            $rules['code'] = ['required', Rule::unique('party_types')->ignore($request->id)];
        }
