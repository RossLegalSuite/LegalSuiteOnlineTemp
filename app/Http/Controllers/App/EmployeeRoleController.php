...
class EmployeeRoleController extends Controller
{
    public function get(Request $request)
    {
        ...
        if ($request->id) {
            $query->where('employee_roles.id', $request->id);
        }

        if ($request->description) {
            $query->where('employee_roles.description', $request->description);
        }

        if ($request->code) {
            $query->where('employee_roles.code', $request->code);
        }
    ...
    public function store(Request $request)
    {
        ...
        if (isset($request->id)) {
            $rules['code'] = ['required', Rule::unique('employee_roles')->ignore($request->id)];
        }
